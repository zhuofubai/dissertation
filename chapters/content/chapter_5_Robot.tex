...
%Embedded systems have become ubiquitous and play an essential role in the upliftment of human society. 
An embedded system is a combination of computer hardware and software, either fixed in capability or programmable, that is specifically designed for a particular function. The behavior of some embedded systems is controlled using software components called "controllers"'. For example, consider a robot that has a high-level goal of moving from one location to another. A reference trajectory will be computed by a path planner. Then at every point of the reference trajectory, the controller control the robot to follow the reference trajectory. The controller of this sort is also a numerical software, because the control code contains many numerical expressions to sense the current state of the system,  reason about the estimates of the state and environment, determine an action to execute and carry it out. But different from the numerical softwares introduced in chapter 3 and chapter 4, the observational data of embedded control systems are time series.
%Typically, a solution to this goal will be computed using a path planner.
...
For BHR, we use models for the kinematics and inverse kinematics developed in prior work \cite{bebek2007whisker} to create a simulation of the robot, implemented in MATLAB Simulink. In the simulations, the dynamics of each of the robot’s degrees of freedom are represented as sixth-order transfer functions. The environment of this robot consists of simulated heart and breathing motions, which the robot arm needs to follow. The BHR system uses a Receding Horizon Model Predictive Control based robotic motion control algorithm. As part of the motion control algorithm, the nonlinearities of the system (i.e., gravitational effects, joint frictions, and Coriolis and centrifugal forces) are also canceled. The controller has a sampling rate of 2 kHz. 
...
We do not build a software system for BHR. The reason is: (1) BHR does not need a delegator to decompose complex tasks, because BHR only has one simple task, which is tracking the heart position. (2) When BHR is running, the user only needs API to PAUSE/RESUME/STOP the robot. These three API are already provided by simulink, so a robot proxy is not necessary. 
...
\begin{algorithm}
  \captionof{figure}{ {\it FLECS 2.0} Algorithm}\label{flecs2.0}
  \begin{algorithmic}[1]
    \Require The software code of control system, simulator, trajectories with both normal and abnormal behavior, and pre-defined constant number $m, n, k$ 
...
In the experiments, we use ESP-SCP and the original version of FLECS as baselines \cite{liang2015fault, liang2013detection}.
...
\label{flbhr}
      ...
      \hline
Bug Index	&	ESP-SCP	&	FLECS	&	FLECS 2.0	\\ \hline
1	&	85	&	73	&	15	\\ \hline
2	&	84	&	24	&	44	\\ \hline
3	&	60	&	97	&	18	\\ \hline
4	&	12	&	24	&	32	\\ \hline
